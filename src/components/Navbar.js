...
function Navbar() {
  return (
    <header className="fixed w-full z-50 transition-colors duration-300 Header">
      <div className="relative flex max-w-screen-xl flex-col overflow-hidden px-4 py-4 text-black md:mx-auto md:flex-row md:items-center">
        ...
        <nav aria-label="Header Navigation" className="flex max-h-0 w-full flex-col items-center justify-between overflow-hidden transition-all peer-checked:mt-8 peer-checked:max-h-56 md:ml-24 md:max-h-full md:flex-row md:items-start">
          <ul className="flex flex-col items-center space-y-2 md:ml-auto md:flex-row md:space-y-0">
            ...
            <li className="md:mr-12"><a href="/contact">Contact</a></li>
            <li className="md:mr-12"><a href="/about">About</a></li>
            <li className="md:mr-12">
              <Link to={'/addhotel'}>
                <button className="book rounded-full border-2 border-black px-6 py-1 text-black transition-colors hover:bg-black hover:text-white">Add Hotels</button>
